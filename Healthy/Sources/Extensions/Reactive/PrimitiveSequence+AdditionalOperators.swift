...
public extension PrimitiveSequence where Trait == MaybeTrait {
    ...
    func flatMap<R>(_ selector: @escaping (E) throws -> Single<R>) -> Maybe<R> {
        return flatMap { try selector($0).asMaybe() }
    }
    
    func flatMapCompletable( _ selector: @escaping (E) throws -> Completable) -> Completable {
        return asObservable()
...
public extension PrimitiveSequence where Trait == SingleTrait {
    ...
    static func combineLatest<T1, T2>(_ source1: Single<T1>,
                                      _ source2: Single<T2>,
                                      resultSelector: @escaping (T1, T2) throws -> ElementType) -> Single<ElementType> {
